...
do
    ...
    sshpass -p SlotRacr ssh $OPT slotracr@$i 'sudo timedatectl set-timezone "America/Chicago"'
    TIME=$(date +"%y-%m-%d %H:%M:%S")
    sshpass -p SlotRacr ssh $OPT slotracr@$i "sudo timedatectl set-ntp false"
    sshpass -p SlotRacr ssh $OPT slotracr@$i "sudo timedatectl set-time '$TIME'"
